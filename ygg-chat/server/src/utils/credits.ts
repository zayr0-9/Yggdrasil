 ...
 */
export function checkCreditsAvailable(userId: number, requiredAmount: number): CreditCheckResult {
  try {
 ...
 */
export function decrementCredits(userId: number, amount: number, reason: string): number | null {
  // Use a transaction to ensure atomicity
  const transaction = db.transaction((uid: number, amt: number, rsn: string) => {
    // Get current balance
    ...
    statements.createCreditLedgerEntry.run(uid, -amt, rsn, newBalance)
 ...
 */
export function replenishCredits(userId: number, tier: 'high' | 'mid' | 'low', reason: string): number {
 ...
 */
export function addCredits(userId: number, amount: number, reason: string): number {
  const transaction = db.transaction((uid: number, amt: number, rsn: string) => {
    // Get current balance
    const result = statements.getUserCredits.get(uid) as { credits_balance: number } | undefined
    const currentBalance = result?.credits_balance ?? 0
 ...
 */
export function getCreditHistory(userId: number, limit: number = 100): CreditTransaction[] {
  try {
 ...
 */
export function getTotalCreditsUsed(userId: number): number {
  try {
