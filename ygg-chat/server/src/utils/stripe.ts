...
export async function handleInvoicePaymentSucceeded(invoice: Stripe.Invoice): Promise<void> {
  ...
  try {
    ...
    if (!subscriptionId) {
      console.log('[Stripe] Invoice payment succeeded but no subscription found, skipping')
      return
    }
...
export async function handleSubscriptionDeleted(subscription: Stripe.Subscription): Promise<void> {
  try {
    ...
    console.log(`[Stripe] Subscription canceled for user ${userId}, access until ${new Date((subscription as any).current_period_end * 1000).toISOString()}`)
  } catch (error) {
 ...
 */
export async function getUserSubscriptionStatus(userId: number): Promise<any> {
  try {
    const user = statements.getUserById.get(userId) as any
